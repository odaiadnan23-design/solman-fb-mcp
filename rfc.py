...
# Tables where RFC_READ_TABLE's fixed-width rendering is known to mislead.
_TRUNCATION_WARNINGS = {
    "CRMD_ORDERADM_H": ("GUID is RAW(16) and renders truncated to 16 hex characters, a prefix "
                        "shared by many documents — join on OBJECT_ID, and take full GUIDs "
                        "from OData."),
...
def _call(fm: str, inner: str) -> str:
...
def read_table(table: str, fields: list[str] | None = None, where: str = "",
    ...
    inner = (f"<DELIMITER>{_xml_escape(delimiter)}</DELIMITER>"
             ...
             f"<DATA></DATA><FIELDS>{field_xml}</FIELDS><OPTIONS>{option_xml}</OPTIONS>")
    body = _call("RFC_READ_TABLE", inner)

    # column order comes back in FIELDS; trust it over the caller's list
    names = re.findall(r"<FIELDNAME>(.*?)</FIELDNAME>", body, re.S) or fields
    names = [n.strip() for n in names]
    rows = []
    for raw in re.findall(r"<WA>(.*?)</WA>", body, re.S):
        parts = [p.strip() for p in _unescape(raw).split(delimiter)]
        rows.append(dict(zip(names, parts)) if names else {"WA": raw})
    return rows
